...
import { UpdatePersonInvoiceDto } from '../person-invoice/dto/update-person-invoice.dto'; // Importe o novo DTO
...
async updatePersonInvoice(
  invoiceId: number,
  personId: number,
  updatePersonInvoiceDto: UpdatePersonInvoiceDto
): Promise<PersonInvoice> {
  const transaction = await this.invoiceModel.sequelize.transaction();
  try {
    const personInvoice = await this.personInvoiceModel.findOne({
      where: { invoiceId, personId },
      transaction,
    });
    ...
    if (!personInvoice) {
      throw new NotFoundException(
        `Association not found for Invoice ID ${invoiceId} and Person ID ${personId}`,
      );
    }

    // Atualiza os campos permitidos
    if (updatePersonInvoiceDto.paid_at !== undefined) {
      personInvoice.paid_at = updatePersonInvoiceDto.paid_at;
    }
    if (updatePersonInvoiceDto.cancelled_at !== undefined) {
      personInvoice.cancelled_at = updatePersonInvoiceDto.cancelled_at;
    }
    if (updatePersonInvoiceDto.enable !== undefined) {
      personInvoice.enable = updatePersonInvoiceDto.enable;
    }

    await personInvoice.save({ transaction }); // Aqui está o problema
    await transaction.commit();
